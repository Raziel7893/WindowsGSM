...
namespace WindowsGSM.Plugins
{
    public class PlanesOfPain : SteamCMDAgent
    {
        ...
        public string Additional = "-ignorecompilererrors -config configs/default.json -userdir ./save-dir"; // Additional server start parameter
        ...
        {
            //removing Port from the cfg so the cmd line parameter sticks
            var dir = ServerPath.GetServersServerFiles(serverData.ServerID);
            var configs = Directory.GetFiles(dir, "*.json");
            foreach (var file in configs)
            {
                ...
                {
                    if(line.Contains("\"port\"") || line.Contains("\"queryPort\"") || line.Contains("\"serverName\""))
                    {
                        continue;
                    }
                    newLines.Add(line);
